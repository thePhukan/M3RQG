...
@dataclass
# class CausalLMOutputWithPast_Value(ModelOutput):
class CausalLMOutputWithPast_Value(CausalLMOutputWithPast):
    """
...
class Phi3DecoderLayer(nn.Module):
    def __init__(self, config: Phi3VConfig, layer_idx: int):
        ...
        self.last_dim_linear = nn.Linear(1024, config.hidden_size) # DINOV2 Large Img Features
        # self.last_dim_linear = nn.Linear(2048, 1024) # ResNext
        
        self.m_attn = MultiHeadAttention(config.hidden_size,8) # MultiheadAttention(embed_dim, num_heads)
    ...
    def forward(
        ...
        **kwargs,
        ...
        #     # print("x.shape before reshaping:", x.shape) #x.shape before reshaping: torch.Size([2, 1, 514, 1408])
        #     # print("x[0].shape before reshaping:", x[0].shape)
        #     y = hidden_states
          
        #     # print("y.shape:", y.shape)
        #     # print("y[0].shape:", y[0].shape) #y.shape: torch.Size([2, 1024, 3072])
        #     # x = self.last_dim_linear(x.to(torch.float16)) # x = [bs, frames, 1024] 
        #     x = self.last_dim_linear(x.to(self.last_dim_linear.weight.dtype))
        #     custom_input, attention_cus = self.m_attn(y,x,x) 
            
        #     # print("x.shape:", x.shape)
            
        #     # # print("attention_cus.shape:", attention_cus.shape)
        #     embed = custom_input + y
...
class Phi3VForCausalLM(Phi3VPreTrainedModel):
    ...
    # Ignore copy
    @add_start_docstrings_to_model_forward(PHI3V_INPUTS_DOCSTRING)
    @replace_return_docstrings(output_type=CausalLMOutputWithPast_Value, config_class=_CONFIG_FOR_DOC)
    def forward(
        self,
        input_ids: torch.LongTensor = None,
        attention_mask: Optional[torch.Tensor] = None,
        ...
        return_dict: Optional[bool] = None,
        video_embedd: Optional[torch.FloatTensor] = None,
    ) -> Union[Tuple, CausalLMOutputWithPast_Value]:
        r"""
        Args:
        ...
        # print("phi3 backend hidden_states:", hidden_states)
        logits = self.lm_head(hidden_states)
        logits = logits.float()

        ##########################################################
        # try:
        value = self.summary(hidden_states).squeeze(-1)
        # except:
        ...
        try:
            ...
            value = value
        ##########################################################
        # print("phi3 backend value:", value)
        ...
        if not return_dict:
            # output = (logits,) + outputs[1:]
            output = (logits,) + outputs[1:] + (value,)
            # print("phi3 backend:", value)
            return (loss,) + output if loss is not None else output
            # return output if loss is not None else output
            # return ((loss,) + output) if loss is not None else output

        # print("check!")
        return CausalLMOutputWithPast_Value(
            loss=loss,
            logits=logits,
            past_key_values=outputs.past_key_values,
            hidden_states=outputs.hidden_states,
